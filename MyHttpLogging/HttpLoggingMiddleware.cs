...
using MyHttpLogging.CustomCode;
...
internal sealed class HttpLoggingMiddleware
{
        ...
        IEnumerable<IHttpLoggingInterceptor> interceptors, ObjectPool<HttpLoggingInterceptorContext> contextPool, TimeProvider timeProvider)
    {
        ...
        //HACK: use my custom logger instead
        ...
        HttpLoggingAttribute? loggingAttribute, HttpLoggingFields loggingFields)
    {
        ...
        if (logContext.IsAnyEnabled(HttpLoggingFields.RequestPropertiesAndHeaders | HttpLoggingFields.RequestQuery))
        {
            ...
            {
                FilterHeaders(logContext, request.Headers, options._internalRequestHeaders, headerPrefix: "request-");      //HACK:
            }
        ...
        try
        {
            ...
            finally
            {
                if (options.CombineLogs)
                {
                    ...
                    {
                        //*********************************************************************************
                        //HACK: Try to force read the request body

                        if (context.Items.ContainsKey("RequestBody"))
                        {
                            var body = context.Items["RequestBody"] ?? "";
                            var entry = KeyValuePair.Create("RequestBody", body);
                            logContext.Parameters.Add(entry);
                        }
    ...
    private static void LogResponseHeadersCore(HttpLoggingInterceptorContext logContext, HttpLoggingOptions options, ILogger logger)
    {
        ...
        {
            FilterHeaders(logContext, response.Headers, options._internalResponseHeaders, headerPrefix: "response-");      //HACK:
        }
    ...
    internal static void FilterHeaders(HttpLoggingInterceptorContext logContext,
        ...
        HashSet<string> allowedHeaders, string headerPrefix)    //HACK: Headerprefix is a hack to be able to separate the request and response headers
    {
        foreach (var (key, value) in headers)
        {
            //HACK; Don't redact the headers
            //if (!allowedHeaders.Contains(key))
            //{
            //    // Key is not among the "only listed" headers.
            //    logContext.AddParameter(headerPrefix + key, Redacted);
            //    continue;
            //}
            logContext.AddParameter(headerPrefix + key, value.ToString());  //HACK: added headerPrefix
        }
    }
}
